...
import type { Monster } from '../components/common/MonsterDetails';
import { MONSTER_TYPES, MONSTER_ATTRIBUTES } from './staticValues';
import type { MonsterType, MonsterAttribute } from './staticValues';

// Re-export for backwards compatibility
export const AVAILABLE_TYPES = MONSTER_TYPES;
export const AVAILABLE_ATTRIBUTES = MONSTER_ATTRIBUTES;

// Berry descriptions
export const BERRY_DESCRIPTIONS: Record<string, string> = {
...
// Pastry descriptions
export const PASTRY_DESCRIPTIONS: Record<string, string> = {
...
export type AvailableType = MonsterType;
export type AvailableAttribute = MonsterAttribute;
export type PastryValueType = 'species' | 'type' | 'attribute' | 'none';

/**
 * Get berry description
 */
export const getBerryDescription = (berryName: string): string => {
 ...
 */
export const getPastryDescription = (pastryName: string): string => {
 ...
 */
export const berryRequiresSpeciesSelection = (berryName: string): boolean => {
 ...
 */
export const getSpeciesSlotAffected = (berryName: string): number => {
 ...
 */
export const canPastryBeUsedOnMonster = (pastryName: string, monster: Monster): boolean => {
  switch (pastryName) {
 ...
 */
export const getPastryValueType = (pastryName: string): PastryValueType => {
 ...
 */
export const getPastryOptions = (pastryName: string): readonly string[] => {
 ...
 */
export const formatMonsterSpecies = (monster: Monster): string => {
 ...
 */
export const getMonsterTypes = (monster: Monster): string[] => {
  return [monster.type1, monster.type2, monster.type3, monster.type4, monster.type5].filter((t): t is string => !!t);
};

/**
 * Get all species from a monster
 */
export const getMonsterSpecies = (monster: Monster): string[] => {
  return [monster.species1, monster.species2, monster.species3].filter((s): s is string => !!s);
};
