...
const FinalFantasyMonsterFormPage = () => {
  const { id } = useParams();
  ...
  // Fetch Final Fantasy monster data if in edit mode
  useEffect(() => {
    if (isEditMode) {
      const fetchMonster = async () => {
        try {
          setLoading(true);
          const response = await monsterTypesApi.getFinalFantasyMonsterById(id);
          setFormData(response.data);
          setImagePreview(response.data.image_url);
        } catch (error) {
          console.error('Error fetching Final Fantasy monster:', error);
          toast.error('Failed to fetch Final Fantasy monster data');
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await monsterTypesApi.updateFinalFantasyMonster(id, monsterData);
        toast.success('Final Fantasy monster updated successfully');
      } else {
        await monsterTypesApi.createFinalFantasyMonster(monsterData);
        toast.success('Final Fantasy monster created successfully');
      }

      // Navigate back to the list page
      navigate('/admin/finalfantasy-monsters');
    } catch (error) {
      console.error('Error saving Final Fantasy monster:', error);
      toast.error(error.response?.data?.message || 'Failed to save Final Fantasy monster');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div>
        <LoadingSpinner message="Loading Final Fantasy monster data..." />
      </div>
    );
  }
          ...
          <div className="admin-form-grid">
            <div className="admin-form-column">
              ...
              <FormSelect
                ...
                onChange={handleChange}
                options={monsterCategories.map(cat => ({ value: cat, label: cat }))}
              ...
              <FormSelect
                ...
                options={[
                  { value: '', label: 'None' },
                  ...ffElements.map(el => ({ value: el, label: el }))
                ]}
              />
              ...
              <FormSelect
                ...
                options={[
                  ...
                  ...ffGames.map(game => ({ value: game, label: game }))
                ]}
              />
            </div>
...
export default FinalFantasyMonsterFormPage;
