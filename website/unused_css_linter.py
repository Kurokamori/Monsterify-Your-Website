...
def extract_js_class_references(js_content: str) -> set[str]:
    """
    ...
    # Pattern 3: Template literals with class names
    template_pattern = re.compile(r'`[^`]*`')
    for match in template_pattern.finditer(js_content):
        template = match.group(0)
        # Extract string parts (non-interpolated)
    ...
    # Pattern 7: Import statements for CSS files (to track which CSS is used)
    # This doesn't extract classes but helps understand the relationship
...
def analyze_unused_classes(
    ...
    for js_file in js_files:
        try:
            ...
            references = extract_js_class_references(content)
    ...
    for class_name in all_css_classes:
        ...
        if class_name not in all_js_references:
...
def main():
    ...
    report = format_report(
        unused_classes, all_css, all_js, script_dir,
