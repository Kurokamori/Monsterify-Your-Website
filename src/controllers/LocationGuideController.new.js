        ...
        if (fs.existsSync(subDirPath)) {
          ...
          try {
            ...
            subLocationFiles.forEach(locationFile => {
              const locationSlug = `${subDir.slice(0, -1)}-${locationFile.replace('.md', '')}`;
...
const LocationGuideController = {
  ...
  showRegions: (req, res) => {
    ...
    const regions = locationData.regions.map(region => ({
      ...region,
      url: `/guides/locations/${region.slug}`
    }));

    res.render('guides/locations/regions', {
  ...
  showAreas: (req, res) => {
    ...
    const areas = getAreasByRegionSlug(regionSlug).map(area => ({
      ...area,
      url: `/guides/locations/${regionSlug}/${area.slug}`
    }));

    res.render('guides/locations/areas', {
  ...
  showLocations: (req, res) => {
    ...
    const locations = getLocationsByAreaSlug(areaSlug).map(location => ({
      ...location,
      url: `/guides/locations/${regionSlug}/${areaSlug}/${location.slug}`
    }));

    res.render('guides/locations/locations', {
  ...
  showLocation: (req, res) => {
    ...
    const { content, metadata } = loadMarkdownContent(markdownPath);

    res.render('guides/locations/location', {
